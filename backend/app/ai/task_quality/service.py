...
def analyze_task_quality_ai(
    ...
    try:
        raw_response = provider.complete(
            prompt=prompt,
            system_prompt=TASK_QUALITY_SYSTEM_PROMPT,
        )
    ...
    try:
        ...
        validated = TaskQualityResponse.model_validate(data)
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="AI provider returned an invalid structured quality assessment. Please try analyzing again.",
        )
